...
int main() {
	...
	for(int i = 0; i < 10; i++) {
		...
		bool xodd;
		bool yodd;
		sx[x.numberOfDigits()-1] % 2 == 0 ? xodd = false : xodd = true;
		sy[y.numberOfDigits()-1] % 2 == 0 ? yodd = false : yodd = true;
		if(!xodd && !yodd) cout << x/2 + y/2 << endl << x/2 - y/2 << endl;
		if(xodd && !yodd) cout << x/2 + y/2 << ".5" << endl << x/2 - y/2 << ".5" << endl;
		if(!xodd && yodd) {
			if(x > 0) cout << x/2 + y/2 << ".5" << endl << x/2 - 1 - y/2 << ".5" << endl;
			else if (x < 0) cout << x/2 + y/2 << ".5" << endl << x/2 + 1 - y/2 << ".5" << endl;
			else cout << x/2 + y/2 << ".5" << endl << x/2 - y/2 << ".5" << endl;
		}
		if(xodd && yodd) cout << x/2 + y/2 + 1 << endl << x/2 - y/2 << endl;
	}
	return 0;
}
