...
typedef pair<be_pair_t,T> m_pair_t;
...
T find_max_profit(T treat[], T begin, T end, T age) {
	map<m_pair_t,T>::iterator it = mem.find(m_pair_t(be_pair_t(begin,end),age));
	if(it != mem.end()) return it->second;
	if(begin == end) {
		mem.insert(pair<m_pair_t,T>(m_pair_t(be_pair_t(begin,end),age),treat[begin]*age));
