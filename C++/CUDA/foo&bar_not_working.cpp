...
__global__ void foo(float out[], float A[], float B[], float C[], float D[], float E[]){
    ...
    if(tid > 1 && tid < N-1)
        out[i] = (local[0][tid] + local[1][tid] + local[2][tid] + local[3][tid] + local[4][tid]) / 5.0f;
    if(tid == N-1 || tid == N)
...
__global__ void bar(float out[], float in[]) 
{
    ...
    if(tid > 1 && tid < N-1)
        out[i] = (local[tid-2] + local[tid-1] + local[tid] + local[tid+1] + local[tid+2]) / 5.0f;

    if(tid == N-1)
        out[i] = (local[tid-2] + local[tid-1] + local[tid] + local[tid+1] + in[i+2]) / 5.0f;
    if(tid == N)
        out[i] = (local[tid-2] + local[tid-1] + local[tid] + in[i+1] + in[i+2]) / 5.0f;
}
