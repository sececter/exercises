...
void computeIteration2(const unsigned char* const dstImg,
...
void computeG2(const unsigned char* const channel,
              float* const g,
              const size_t numColsSource,
              const std::vector<uint2>& interiorPixelList)
{
...
void reference_calc2(const uchar4* const h_sourceImg,
  ...
  cout << "interiorPixelList.size: "<< interiorPixelList.size() << endl;
  ...
  computeG2(red_src,   g_red,   numColsSource, interiorPixelList);
  computeG2(blue_src,  g_blue,  numColsSource, interiorPixelList);
  computeG2(green_src, g_green, numColsSource, interiorPixelList);
  ...
  const size_t numIterations = 680;
  for (size_t i = 0; i < numIterations; ++i) {
    computeIteration2(red_dst, strictInteriorPixels, borderPixels,
  ...
  for (size_t i = 0; i < numIterations; ++i) {
    computeIteration2(blue_dst, strictInteriorPixels, borderPixels,
  ...
  for (size_t i = 0; i < numIterations; ++i) {
    computeIteration2(green_dst, strictInteriorPixels, borderPixels,
                ...
                uchar4* const h_blendedImg) //OUT
{
 ...
 cout << "numRowsSource: " << numRowsSource << endl;
 cout << "numColsSource: " << numColsSource << endl;

    reference_calc2(h_sourceImg, numRowsSource, numColsSource,
