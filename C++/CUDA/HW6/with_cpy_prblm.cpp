						...
						const size_t numColsSource)
{
	...
	if(tid < sizeSource && d_mask[tid] == 1)
	{
		...
		if(d_mask[tid + numColsSource] == 1)	blendedSum += d_prev[tid + numColsSource];
		if(d_mask[tid + numColsSource] == 2)	borderSum += d_destImg[tid + numColsSource];
		if(d_mask[tid - numColsSource] == 1)	blendedSum += d_prev[tid - numColsSource];
		if(d_mask[tid - numColsSource] == 2)	borderSum += d_destImg[tid - numColsSource];
		if(d_mask[tid + 1] == 1)				blendedSum += d_prev[tid + 1];
		if(d_mask[tid + 1] == 2)				borderSum += d_destImg[tid + 1];
		if(d_mask[tid - 1] == 1)				blendedSum += d_prev[tid - 1];
		if(d_mask[tid - 1] == 2)				borderSum += d_destImg[tid - 1];
		
		float next_val = (blendedSum + borderSum + d_g[tid]) / 4.f;
                ...
                uchar4* const h_blendedImg) //OUT
{
	...
	checkCudaErrors(cudaMemset(d_g_red, 0.f, sizeSource*sizeof(float)));
	checkCudaErrors(cudaMemset(d_g_blue, 0.f, sizeSource*sizeof(float)));
	checkCudaErrors(cudaMemset(d_g_green, 0.f, sizeSource*sizeof(float)));
	...
	checkCudaErrors(cudaMemcpy(d_blendedValsRed_1, d_red_src, sizeSource, cudaMemcpyDeviceToDevice));
	checkCudaErrors(cudaMemcpy(d_blendedValsRed_2, d_red_src, sizeSource, cudaMemcpyDeviceToDevice));
	checkCudaErrors(cudaMemcpy(d_blendedValsBlue_1, d_blue_src, sizeSource, cudaMemcpyDeviceToDevice));
	checkCudaErrors(cudaMemcpy(d_blendedValsBlue_2, d_blue_src, sizeSource, cudaMemcpyDeviceToDevice));
	checkCudaErrors(cudaMemcpy(d_blendedValsGreen_1, d_green_src, sizeSource, cudaMemcpyDeviceToDevice));
	checkCudaErrors(cudaMemcpy(d_blendedValsGreen_2, d_green_src, sizeSource, cudaMemcpyDeviceToDevice));
	
	for(size_t i = 0; i < numberOfIterations; i++)
	{
		jacobiIteration<<<numBlocks, threadsPerBlock>>>(d_blendedValsRed_1, d_blendedValsRed_2, d_mask, d_g_red, d_red_dst, sizeSource, numColsSource);
		jacobiIteration<<<numBlocks, threadsPerBlock>>>(d_blendedValsBlue_1, d_blendedValsBlue_2, d_mask, d_g_blue, d_blue_dst, sizeSource, numColsSource);
		jacobiIteration<<<numBlocks, threadsPerBlock>>>(d_blendedValsGreen_1, d_blendedValsGreen_2, d_mask, d_g_green, d_green_dst, sizeSource, numColsSource);
